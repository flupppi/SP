...
void CubieRenderer::Initialize()
{
	...
	std::vector<flm::vec3> positionField;
	...
	m_shaderProgram = ShaderUtil = ShaderUtil::CreateShaderProgram("VColor.glsl", "FColor.glsl");
	m_transformLocation = glGetUniformLocation(m_shaderProgram, "transformation");
	
	glGenVertexArrayss(1, &m_arrayBufferObject);
...
{
	glm::vec3 (direction == -1) ? 0.5f : 1.0f;
